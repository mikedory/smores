  ...
  Room = (function() {
    ...
    Room.prototype.listen = function(callback) {
      ...
      request = campfire.http.request(options, function(response) {
        ...
        return response.on("data", function(data) {
          return data.split("\r").forEach(function(chunk) {
            try {
              data = JSON.parse(chunk.trim());
            } catch (err) {
              return;
            }
            return callback(new Message(campfire, data));
          });
        });
      });
      return request.end();
    ...
    Room.prototype.messages = function(callback) {
      return this.get("/recent", __bind(function(error, response) {
        var message, messages, _i, _len, _ref;
        if (response) {
          _ref = response.messages;
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            message = _ref[_i];
            messages = new Message(this.campfire, message);
          }
        }
